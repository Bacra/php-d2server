...
class Handlers
{
	...
	RulesStringValue(19,"Chrome", "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.17 (KHTML, like Gecko) Chrome/24.0.1312.52 Safari/537.17")
	...
	public static RulesOption("qu.8684.com", "autoMapToLocalFile")
	var autoMapToLocalFile4square: boolean = true;
	public static RulesOption("shenzhen.8684.com", "autoMapToLocalFile")
	var autoMapToLocalFile4localSZ: boolean = true;
	public static RulesOption("beijing.8684.com", "autoMapToLocalFile")
	var autoMapToLocalFile4localBJ: boolean = true;
    public static RulesOption("passport.8684.com", "autoMapToLocalFile")
    var autoMapToLocalFile4passport: boolean = true;

	// 根据目录，自动判断文件是否需要转发
	static function autoMapToLocalFile(oSession: Session, domain: String, localPath: String, dealLinkFn: Function){
		if (domain == oSession.host) {
			var index:int = oSession.PathAndQuery.indexOf('?');
			var path:String = oSession.PathAndQuery;
			if (index != -1) path = path.Substring(0, index);
			if (path.EndsWith('/')) path = path + 'index.html';
			if (path.indexOf('.') == -1) path = path + '.html';
			var reg:RegExp = new RegExp('(.js|.css|.less|.html|.png|.jpg|.gif)$');
			if (reg.test(path)) {
				path = dealLinkFn ? dealLinkFn(localPath, path) :  localPath + path;
				// path = localPath + path;
				...
				if (System.IO.File.Exists(path)) {
					...
					// FiddlerObject.log('>>Folder reDirect<< ' + path);
				}
			}
		}
	}
	...
	static function OnBeforeRequest(oSession: Session) {
		...
		/*FiddlerObject.log(oSession.host);
		FiddlerObject.log(oSession.hostname);
		FiddlerObject.log(oSession.PathAndQuery);
		if (oSession.host.indexOf('google') != -1 && oSession.host.indexOf('gstatic.com') != -1) {
			oSession.host = '74.125.135.18';
		}*/

		if (autoMapToLocalFile4square) {
			autoMapToLocalFile(oSession, 'qu.8684.com', 'W:/8684square/', null);
		}
		if (autoMapToLocalFile4localSZ) {
			autoMapToLocalFile(oSession, 'shenzhen.8684.com', 'T:/', function(localPath: String, path: String){
				return localPath+path.replace('.min', '');
			});
		}
		if (autoMapToLocalFile4localBJ) {
			autoMapToLocalFile(oSession, 'beijing.8684.com', 'T:/', function(localPath: String, path: String){
				return localPath+path.replace('.min', '');
			});
		}
        if (autoMapToLocalFile4passport) {
            autoMapToLocalFile(oSession, 'passport.8684.com', 'w:/8684passport/', function(localPath: String, path: String){
                return localPath+path.replace('.min', '');
            });
        }
